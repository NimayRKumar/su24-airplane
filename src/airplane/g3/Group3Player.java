...
public class Group3Player extends airplane.sim.Player {
    ...
    public void startNewGame(ArrayList<Plane> planes) {
        ...
        divergentIndices = getDivergentPlanes(planes);
    }
    ...
    private boolean[] getDivergentPlanes(ArrayList<Plane> planes) {
        ArrayList<Plane> divergentPlanes = new ArrayList<>();
        ...
        for (int i = 0; i < planes.size() - 1; ++i) {
            for (int j = i+1; j < planes.size(); ++j) {
                Plane pi = planes.get(i);
                Plane pj = planes.get(j);
                double dist = Math.sqrt(Math.pow(pi.getX() - pi.getY(), 2.0) + Math.pow(pj.getX() - pj.getY(), 2.0));

                //if planes have same destination and neither are already landed
                //if (pi.getLocation().distance(pj.getLocation()) <= GameConfig.SAFETY_RADIUS && pi.getBearing()==-1 && pj.getBearing()==-1) {
                if (dist <= GameConfig.SAFETY_RADIUS && pi.getBearing()==-1 && pj.getBearing()==-1) {
                    //System.err.println("Plane 1: " + pi.getLocation());
                    //System.err.println("Plane 2: " + pj.getLocation());
                    //System.err.println("distance :" + i+"= " + dist);
                    //divergentPlanes.add(pi);
                    divergentIndices[i] = true;
                    //pi.setDepartureTime(pi.getDepartureTime()+i*1000);
                    //divergentIndices[j] = true;
                }
            }
        }

        sortPlanesByDeparture(divergentPlanes);
